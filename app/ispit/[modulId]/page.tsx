...
export default function IspitPage() {
  ...
  useEffect(() => {
    ...
    const checkLock = (currentModulId: number, hasMedals: string[]) => {
      if (currentModulId === 2 && !hasMedals.includes('modul1')) return true;
      if (currentModulId === 3 && !hasMedals.includes('modul2')) return true;
      return false;
    };
    ...
    const unsub = onAuthStateChanged(firebaseAuth, async (u) => {
      ...
      if (u) {
        ...
        if (uSnap.exists()) {
          const stats = uSnap.data();
          setUserStats(stats);
          if (checkLock(Number(modulId), stats.medalje || [])) {
            setIsLocked(true); setLoading(false); return;
          }
        }
      }
      try {
        const p = await fetchPitanjaByModul(Number(modulId));
        if (!p || p.length === 0) setError("Nema pitanja.");
        else setPitanja(p);
      } catch (e) { setError("Greška na serveru."); }
  ...
  const handleZavrsiIspit = async (final = rezultati) => {
    ...
    if (user) {
      const m = await spremiIspitSustav(user, { modulId, ukupnoBodova: ukupno, postotak, vrijemeTrajanja: trajanje, email: user.email });
      setNoveMedalje(m);
    }
  ...
  if (loading || isSaving) return (
    <div className="h-screen flex flex-col items-center justify-center bg-white p-6 text-center">
      ...
      <h2 className="font-black text-gray-300 uppercase tracking-tighter italic">{isSaving ? "Analiziramo tvoj uspjeh..." : "Provjera pristupa..."}</h2>
    </div>
  );
  ...
  if (isLocked) {
    return (
      <div className="h-screen flex items-center justify-center p-6 bg-gray-50">
        <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl text-center border">
          <div className="w-20 h-20 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-6"><Lock size={32} /></div>
          <h2 className="text-3xl font-black text-gray-900 mb-4 uppercase italic tracking-tighter">Zaključano</h2>
          <p className="text-gray-500 font-bold mb-10 leading-relaxed italic text-sm">Položi prethodni modul kako bi otključao ovaj ispit.</p>
          <button onClick={() => router.push('/')} className="w-full bg-gray-900 text-white py-5 rounded-2xl font-black uppercase flex items-center justify-center gap-3 active:scale-95 transition-all"><ArrowLeft size={18} /> Povratak</button>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto my-6 md:my-10 p-4">
        <div className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl border p-6 md:p-10 text-center relative overflow-hidden">
              ...
              <div className="flex flex-wrap gap-4 bg-gray-50 p-6 rounded-[2.5rem] border border-gray-100 shadow-inner relative">
                {sec.data.map(m => (
                  <div key={m} className="group relative">
                    ...
                    {/* Tooltip fiksiran za lijevi rub medalje da ne izlazi van ekrana */}
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-10">
            <button onClick={() => window.location.reload()} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase shadow-xl active:scale-95 transition-all">Ponovi Ispit</button>
        ...
        <div className="bg-white w-full max-w-2xl rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-16 shadow-2xl text-center relative overflow-hidden animate-in fade-in zoom-in duration-500">
          ...
          <div className="space-y-3 mb-10 text-left">
            <div className="bg-blue-50 p-4 md:p-5 rounded-2xl md:rounded-3xl border border-blue-100 flex items-center gap-4">
              ...
              <p className="font-black text-blue-900 italic text-sm md:text-base">Vrijeme rješavanja ispita: 45 minuta</p>
            </div>
            <div className="bg-green-50 p-4 md:p-5 rounded-2xl md:rounded-3xl border border-green-100 flex items-center gap-4">
              ...
              <p className="font-black text-green-900 italic leading-tight text-xs md:text-sm">Za prolaz na ispitu potrebno je 90% točnih odgovora.</p>
            </div>
            <div className="bg-orange-50 p-4 md:p-5 rounded-2xl md:rounded-3xl border border-orange-100 flex items-center gap-4">
              ...
              <p className="font-black text-orange-900 italic leading-tight text-xs md:text-sm">Nema povratka na prethodna pitanja. Svaki odgovor je konačan.</p>
            </div>
          </div>

          <button onClick={kreniSIspitom} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-6 md:py-8 rounded-[1.5rem] md:rounded-[2.5rem] font-black text-xl md:text-2xl uppercase tracking-widest active:scale-95 transition-all flex items-center justify-center gap-3"><Play fill="currentColor" size={24} /> ZAPOČNI ISPIT</button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto p-4 min-h-screen flex flex-col justify-center">
      <div className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl border relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-6 md:mb-10 text-[10px] md:text-xs font-black text-gray-300 uppercase italic">
          <span>Modul {modulId} • {indeks + 1}/{pitanja.length}</span>
        ...
        <div className="grid gap-3">
          {['A', 'B', 'C'].map(s => (
            <button key={s} onClick={() => setOdabrano(s)} className={`w-full text-left p-4 md:p-6 border-2 rounded-[1.5rem] flex items-center gap-4 transition-all ${odabrano === s ? 'border-blue-600 bg-blue-50' : 'border-gray-50'}`}>
        ...
        <button onClick={handleSljedece} disabled={!odabrano} className="w-full mt-8 bg-gray-900 text-white py-6 rounded-[1.5rem] font-black uppercase flex items-center justify-center gap-3 active:scale-95 transition-all disabled:opacity-20 shadow-xl">
          {indeks === pitanja.length - 1 ? 'Završi Ispit' : 'Sljedeće Pitanje'} <ChevronRight />
        </button>
      </div>
